...
func (n1 Node) CompareWith(n2 ElementWithPriority) int {
	n := n2.(Node)
	if n1.dist < n.dist {
		return -1
	}
...
func dijkstra(start int, graph map[int][]Node, nodes int) []int {
	pq := PriorityQueue{}
	dist := []int{}
	for idx := 0; idx < nodes; idx++ {
		dist = append(dist, 1e9)
	}
	dist[start] = 0
	...
	for len(pq) > 0 {
		first := pq.Remove()
		node := (*first).(Node)
		if node.dist != dist[node.node] {
			continue
		}

		for _, adj := range graph[node.node] {
			if dist[adj.node] > dist[node.node]+adj.dist {
				dist[adj.node] = dist[node.node] + adj.dist
				pq.Insert(Node{adj.node, dist[adj.node]})
			}
		}
	}
	return dist
...
func DijkstrasAlgorithm(start int, edges [][][]int) []int {
	...
	for idx := 0; idx < len(edges); idx++ {
		for _, edges := range edges[idx] {
			node := edges[0]
			cost := edges[1]
			graph[idx] = append(graph[idx], Node{node: node, dist: cost})
		}
	}

	dist := dijkstra(start, graph, len(edges))
